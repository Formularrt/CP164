...
class Sorted_Set:
    ...
    def __eq__(self, target):
        ...
        if len(self._values) != len(target._values):
            ...
            for key in self._values:
                index = target._binary_search(key)
                if index == -1:
                    equals = False
    ...
    def issuperset(self, target):
        ...
        while i < len(target._values) and superset == True:
            key = target._values[i]
    ...
    def isdisjoint(self, target):
        ...
        while i < len(target._values) and i < len(self._values) and disjoint == True:
            key = target._values[i]
    ...
    def intersection(self, target):
        ...
        for value in self._values:
            if target._binary_search(value) != -1:
                new_set._add(value)
    ...
    def symmetric_difference(self, target):
        ...
        while i < len(self._values):
            key = self._values[i]
            if target._binary_search(key) == -1:
                new_set._add(key)
            i += 1

        i = 0
        while i < len(target._values):
            key = target._values[i]
            if self._binary_search(key) == -1:
                new_set._add(key)
            i += 1
